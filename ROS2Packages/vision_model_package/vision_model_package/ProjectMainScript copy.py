...
class XYZPublisher(Node):
    def __init__(self, xyz_coordinates_list):
        super().__init__('xyz_coordinates_publisher')
        ...
        self.xyz_coordinates_list = xyz_coordinates_list

    def timer_callback(self):
        msg = Float32MultiArray()
        msg.data = np.array(self.xyz_coordinates_list).flatten().tolist()
...
def main():
    ...
    # Create the XYZPublisher node and pass the xyz_coordinates_list to it
    xyz_publisher = XYZPublisher(xyz_coordinates_list)

    # Spin the node so the timer_callback function gets called
